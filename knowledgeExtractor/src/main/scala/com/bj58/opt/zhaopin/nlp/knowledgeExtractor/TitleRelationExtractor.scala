...
/**
  * Created by Cheney WANG. 2018/06/28
  */
...
object TitleRelationExtractor {
  ...
  def RelationExtractor(line:String):String = {
    ...
    for(ele_1 <- Arr)
    {
      for(ele_2 <- Arr)
      {
        ...
        {
          val NewEntity = "Relation"+ ele_1.split("\1")(1).toUpperCase + "To" + ele_2.split("\1")(1).toUpperCase + ":\t" +ele_1 + "\t-\t" + ele_2
  ...
  def main(args : Array[String]):Unit ={
    val conf = new SparkConf().setAppName("TitleExtractor")
    ...
    val FirstResult = mergedCutRes.filter(line => line.split("\t")(3).split("\2").length>1).map(line => line.split("\t")(3))
    ...
    val FifthResult = ForthResult.flatMap(line => line.split("\3")).filter(line => !line.isEmpty)
